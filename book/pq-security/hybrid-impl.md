...
- ลดความเสี่ยงจากการโจมตี HNDL (Mitigation of HNDL): PQC มีบทบาทสำคัญในการลดความเสี่ยงจากการโจมตีลักษณะ Harvest-Now, Decrypt-Later (HNDL) โดยเฉพาะข้อมูลที่ต้องจัดเก็บและปกป้องในระยะยาว
...
การเปลี่ยนผ่านระบบรหัสลับไปสู่ยุควอนตัม (Post-Quantum Cryptography: PQC) อาจเริ่มต้นด้วยการใช้โพรโตคอลระบบรหัสลับแบบผสม (PQC-Classical Hybrid Protocols) ระหว่างระบบรหัสลับแบบดั้งเดิมที่เปราะบางต่อการโจมตีจากคอมพิวเตอร์ควอนตัม (Quantum-vulnerable) และแบบใหม่ที่มีความทนทาน (Quantum-resistant) ทั้งสำหรับการสร้างกุญแจรหัสลับร่วมกัน (Key Establishment) และการลงลายมือชื่อดิจิทัล (Digital Signature) 

ระบบรหัสลับแบบผสมสามารถนำมาใช้ เพื่อป้องกันความเสี่ยงจากข้อบกพร่องหรือข้อผิดพลาดในการใช้งานในอัลกอริทึมใดอัลกอริทึมหนึ่ง นอกจากนี้ ระบบรหัสลับแบบผสมยังสามารถนำมาใช้ในกรณี หรือ อุตสาหกรรมที่ยังมีความจำเป็นต้องใช้อัลกอริทึมแบบดั้งเดิมในระหว่างช่วงการเปลี่ยนผ่านนี้
...
ทั้งนี้ ไม่ว่าระบบรหัสลับแบบผสมจะถูกนำไปใช้งานจริงหรือไม่ อัลกอริทึมระบบรหัสแบบดั้งเดิมและแบบยุคควอนตัม จะถูกใช้งานควบคู่กันในหลายแอปพลิเคชันและระบบระหว่างการเปลี่ยนผ่านไปสู่ยุคควอนตัม ตัวอย่างเช่น โปรโตคอลความปลอดภัยของเครือข่ายหลายตัวรองรับการใช้งานชุดอัลกอริทึมระบบรหัสหลายชุดและอนุญาตให้คู่สื่อสารเจรจาเลือกใช้อัลกอริทึมที่ต้องการในแต่ละเซสชันได้ โดยไม่ถือว่าเป็นระบบรหัสแบบผสม เพราะว่าแต่ละเซสชันใช้อัลกอริทึมระบบรหัสเพียงชุดเดียว
...
กุญแจรหัสลับที่ถูกสร้างจากระบบรหัสลับสำหรับยุคควอนตัมตามที่ระบุในมาตรฐาน FIPS 203 สามารถนำมาใช้เป็นองค์ประกอบหนึ่งร่วมกันกับระบบรหัสลับแบบดั้งเดิมได้ ทำให้กุญแจผลลัพธ์มีความปลอดภัยจากการโจมตีด้วยคอมพิวเตอร์ควอนตัม

ในปัจจุบัน Internet Engineering Task Force (IETF) กำลังพัฒนามาตรฐานการสร้างกุญแจรหัสลับร่วมกันแบบผสม สำหรับ Transport Layer Security (TLS) 1.3
...
เช่น การผสมผสานอัลกอริทึม Elliptic Curve Diffie-Hellman Ephemeral (ECDHE) ซึ่งเป็นอัลกอริทึมแบบดั้งเดิม เข้ากับ ML-KEM ซึ่งเป็นอัลกอริทึม PQC เพื่อให้การสื่อสารปลอดภัยทั้งจากคอมพิวเตอร์ปัจจุบันและคอมพิวเตอร์ควอนตัมในอนาคต โดยมีรายละเอียดโดยสังเขปดังนี้

```{figure} ../figures/ecdhe-kem.png
---
name: ecdhe-kem
---
ตัวอย่างการสร้างกุญแจรหัสลับร่วมกันแบบผสมด้วย ECDHE และ MLKEM
```
- **Client ส่งกุญแจสาธารณะ 2 ชุด** ไปยัง Server ได้แก่ ECDHE ephemeral public key และ ML-KEM public key
- **Server สร้างกุญแจลับ** โดยคำนวณ ECDHE share และทำการ ML-KEM encapsulation จากนั้นส่ง ECDHE share และ ML-KEM ciphertext กลับมาให้ Client
- **ทั้งสองฝ่ายได้กุญแจลับ 2 ชุดแยกกัน** ได้แก่ ECDHE secret (จาก ECDH) และ ML-KEM secret (จากการ decapsulate)
- **รวมกุญแจด้วย KDF** นำ ECDHE secret และ ML-KEM secret มาผสมกันผ่าน Key Derivation Function (KDF) เพื่อสร้าง Shared Secret สุดท้ายที่ใช้เข้ารหัสการสื่อสาร
...
### ใบรับรองดิจิทัลแบบผสม

ใบรับรองดิจิทัลแบบผสม (Hybrid Digital Certificate) เป็นองค์ประกอบสำคัญในการใช้งาน การสร้างกุญแจรหัสลับร่วมกันแบบผสม และ การลงลายมือชื่อดิจิทัลแบบผสม โดยรวมอัลกอริทึมแบบดั้งเดิมและแบบ PQC ไว้ใน ใบรับรองดิจิทัล X.509 เดียวกัน เพื่อให้ยังคงใช้งานร่วมกับระบบเดิมได้ ขณะเดียวกันก็เพิ่มความมั่นคงปลอดภัยในระยะยาว ทำให้มีความเหมาะกับการใช้งานในระยะยาว (long-lived credentials) โดยใบรับรองดิจิทัลแบบผสมสามารถแบ่่งเป็นประเภทต่าง ๆ ได้ดังนี้ {cite:p}`ricchizzi2025applied`

- __Pure PQC Certificate__: ใบรับรองที่มีเฉพาะกุญแจสาธารณะและลายเซ็นแบบ Post-Quantum เท่านั้น ไม่มีการสำรองกลับไปใช้อัลกอริทึมแบบดั้งเดิม
- __Parallel Certificate__ Chains: ออกและตรวจสอบใบรับรองแบบดั้งเดิมและแบบ PQC แยกจากกัน เป็นสองสายโซ่ใบรับรองที่ทำงานคู่ขนานกัน
- __Hybrid Certificate__: โครงสร้างใบรับรองแบบดั้งเดิม แต่ฝังกุญแจและลายเซ็นแบบ PQC ไว้ในส่วนขยายทางเลือก (alternative extensions)
- __Composite Certificate__: รวมกุญแจแบบดั้งเดิมและแบบ PQC เข้าเป็นกุญแจสาธารณะแบบผสมหนึ่งชุด และมีลายเซ็นแบบผสมที่ประกอบด้วยทั้งสองอัลกอริทึม
- __Chameleon Certificate__: ใบรับรองแบบดั้งเดิมที่ขยายเพิ่มเติมด้วยตัวบ่งชี้ Delta Certificate เพื่อให้สามารถสร้างใบรับรองเทียบเท่าแบบ PQC ได้เชิงอัลกอริทึม ช่วยรักษาความเข้ากันได้ย้อนหลังพร้อมเตรียมความพร้อมสู่ PQC

```{figure} ../figures/hybrid-certificate.png
---
